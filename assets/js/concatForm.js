...
document.addEventListener('DOMContentLoaded', () => {
    // Get the ul that holds the collection of tags
    const $offerVariationsCollectionHolder = document.querySelector('ul.offerVariations');
    // count the current form inputs we have (e.g. 2), use that as the new
    // index when inserting a new item (e.g. 2)
    const $index = $offerVariationsCollectionHolder.querySelectorAll('input').length;
    $offerVariationsCollectionHolder.setAttribute('index', $index);
